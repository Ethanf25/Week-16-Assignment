...
const TaskCard: React.FC<TaskCardProps> = ({ tasks, onDelete, onToggle, onEdit }) => { // I then used the TaskCardProps interface to type the props of the TaskCard component
  ...
  const startEditing = (task: Task) => { // function to start editing a task
    ...
    setEditText(task.name); // and pre-fill the editText with the current task name
  };

  const saveEdit = (id: number) => { // this is the next function to save the edited task
    if (editText.trim()) { // this line will only save if the editText is not just whitespace
      ...
      setEditingId(null); // this exits editing mode
    }
  };

  const cancelEdit = () => { // Cancel editing function
    setEditingId(null); // this exits editing mode
  };
        ...
        <div className="item-card" key={task.id}> {}
          {editingId === task.id ? ( // this will then check if the current task is being edited
            <> {}
              ...
              />
              <button onClick={() => saveEdit(task.id)}>Save</button> {/* Button to save edited task */}
...
export default TaskCard; // Export the TaskCard component so it can be used in other files
